...
def load_command(name: str) -> Callable:
    def _load() -> type[Command]:
        ...
        command_class = getattr(module, "".join(c.title()
                                for c in words) + "Command")
...
class Application(BaseApplication):
    ...
    def create_io(
        ...
        formatter.set_style("c2_dark", Style(
            "default", options=["bold", "dark"]))
    ...
    def _configure_installer(self, command: InstallerCommand, io: IO) -> None:
        ...
        installer.use_executor(poetry.config.get(
